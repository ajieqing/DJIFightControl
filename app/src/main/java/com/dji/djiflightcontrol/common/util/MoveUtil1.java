    ...
    public void startAction(Action action) {
        ...
        switch (type) {
            ...
            case Down:
                ...
                break;
            case Left:
                bsa=new BaseStartAction(Math.sqrt(x * x + y * y));
                mPitch = -0.2f;
                break;
            case Right:
                bsa=new BaseStartAction(Math.sqrt(x * x + y * y));
...
//                                failureMedios.add(new MyMedio(getH(), la, lo, Setting.NAME));
    ...
    private class BaseStartAction extends BeforeStartAction {
        ...
        public BaseStartAction(double distance) {
            this.distance = distance;
        }

        @Override
        void actionStart() {
            if (MoveUtil1.this.action != null) {
                n = (int) (Math.abs(distance) / d);
                if (n == MoveUtil1.this.action.getN()) {
                    takePhoto();
                    carrect = 0;
                    ...
                    MoveUtil1.this.action = null;
                    MoveUtil1.this.bsa = null;
                } else if (n == MoveUtil1.this.action.getStep()) {
                    MoveUtil1.this.action.setStep(n + 1);
                    takePhoto();
                }
            }
        }
